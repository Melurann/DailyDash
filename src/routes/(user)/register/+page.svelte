		...
		<form use:enhance class="flex flex-col gap-4" method="POST">
			...
			<Field {form} name="confirmPassword">
				<fieldset class="flex flex-col gap-2">
					<Control>
						{#snippet children({ props })}
							<Label class="text-sm font-medium {$errors.confirmPassword ? 'text-red-700' : ''}"
							...
							<input
								...
								type="password"
								placeholder="********"
								class="rounded-md text-sm {$errors.confirmPassword ? 'border-red-700' : ''}"
			...
			<p class="text-sm">
				Already have an account? <a class="text-primary" href="/login">Sign in to your account</a>
			</p>
		</form>
	</div>
</div>
