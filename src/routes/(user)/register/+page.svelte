		...
		<div class="w-full p-6">
			<form on:submit|preventDefault={handleSubmit} class="flex flex-col space-y-4">
				...
				<fieldset class="space-y-2">
					...
					<input
						bind:value={email}
						class="flex h-10 w-full rounded-md border border-input px-3 py-2 text-sm focus:outline-none focus:ring-input"
						id="email"
						name="email"
						type="email"
						aria-required="true"
                ...
                <a href="/login">Already have an account? <span class="text-primary font-medium">Sign in to your account</span></a>
			</form>
		</div>
	</div>
</div>